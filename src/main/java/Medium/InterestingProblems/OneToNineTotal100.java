...
public class OneToNineTotal100 {
    ...
    public static void main(String[] args) {
        ...
        for (String each : temp   ) {  // this loop will send the string to check if it is equals 100 or not
            if(check(each))
            System.out.println(each+" = 111");
        }
    }
    public static boolean check(String arg){// 12-3+45-6+789 given strings and after splitting from +/-
        ...
        for (String pos_split : arg.split("\\+")) {// given string'll be split on "+" sign=> [12-3,45-6,789]
            ...
            for (String neg_split : pos_split.split("-")) {  // [12,3],[45,6],[789]
                int splittedInt = n!=0? Integer.parseInt(neg_split)*(-1):Integer.parseInt(neg_split);
                n++;    // if it is not first string after negative number splitting it will multiply with -1 after parsed to Integer  
                sum += splittedInt; // it will be added to sum
            }
        }
        return sum==111  ;  // true / false
    }
}
